...
$count = 0;
$list =[];
$sql = "SELECT id FROM speciality WHERE id IN ( SELECT id FROM user_speciality WHERE user_speciality.idSpeciality=speciality.id)";
...
$list2 =[];
$sql = "SELECT id FROM speciality WHERE id IN ( SELECT id FROM mission_speciality WHERE mission_speciality.id_Speciality=speciality.id)";
$query = $dbConnect->prepare($sql);
$query->execute();
foreach ($query->fetchAll(PDO::FETCH_NUM) as $tabValues) {
  foreach ($tabValues as $tabElement){
  ...
  <td class="text-center px-4 py-2 fs-5"><?php echo  $title ?></td>
                ...
                <td class="text-center">
                  <a class="btn btn-success" href="../update/speciality_update.php?id=<?php echo $id ?>"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil-fill" viewBox="0 0 16 16">
                    ...
                    <button  type="button" data-bs-toggle="modal" <?php if(in_array($row["id"],$list)) {echo "disabled";}  elseif(in_array($row["id"],$list2)) {echo "disabled";} ?>  data-bs-target="#deleteModal<?php echo $count ?>" class="btn btn-danger" href="../update/delete.php?idSpeciality=<?php $row["id"] ?>"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash3-fill" viewBox="0 0 16 16">
